...
namespace Studio23.SS2.UPMAssistant.Editor
{
    public class PackageJsonWindowController : EditorWindow
    {
        ...
        {
            var window = GetWindow<PackageJsonWindowController>(true, "Generate package.json");
        ...
        {
            var packageNameData = DataHandler.GetSavedPackagedName();
            var filePath = Path.Combine(DataHandler.Root, packageNameData, DataHandler.PackageJson);
        ...
        private void OnGUI()
        {
            ...
            foreach (var scopedRegistry in _jsonData.ScopedRegistries)
            {
                ...
                for (var i = 0; i < scopedRegistry.Scopes.Count; i++)
            ...
            for (var i = 0; i < _jsonData.Dependencies.Count; i++)
            {
                EditorGUILayout.BeginHorizontal();
                GUILayout.Space(20); // Add left padding

                var dependency = _jsonData.Dependencies.ElementAt(i);
                var dependencyName = EditorGUILayout.TextField("Name:", dependency.Key);
                var dependencyVersion = EditorGUILayout.TextField("Version:", dependency.Value);

                var itemsToUpdate = new List<KeyValuePair<string, string>>();
            ...
            for (var i = 0; i < _jsonData.Keywords.Count; i++)
            {
                EditorGUILayout.BeginHorizontal();
                GUILayout.Space(20); // Add left padding
                _jsonData.Keywords[i] = EditorGUILayout.TextField($"Keyword {i + 1}:", _jsonData.Keywords[i]);
                if (GUILayout.Button("Remove", GUILayout.Width(80))) _jsonData.Keywords.RemoveAt(i);
                EditorGUILayout.EndHorizontal();
            }

            if (GUILayout.Button("Add Keyword", GUILayout.Width(150))) _jsonData.Keywords.Add("");
            ...
            if (GUILayout.Button("Generate", GUILayout.Height(35))) PackageHandler.SaveData(_jsonData);
