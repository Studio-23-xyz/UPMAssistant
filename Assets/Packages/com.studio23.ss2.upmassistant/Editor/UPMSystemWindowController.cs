...
namespace Studio23.SS2.UPMAssistant.Editor
{
    public class UPMSystemWindowController : EditorWindow
    {
        ...
        {
            var window = GetWindow<UPMSystemWindowController>();
        ...
        private void OnGUI()
        {
            ...
            {
                if (_packageName != "") DataHandler.SavePackageNameData(_packageName);
                Refresh();
            }
            ...
            if (GUILayout.Button("Delete System", GUILayout.Width(position.width / 4 - 5)))
            {
                ...
                {
                    var userConfirmed = EditorUtility.DisplayDialog("Delete Folder",
                        "Are you sure you want to delete the folder and its contents?", "Yes", "No");
            ...
            _packageName =
                EditorGUILayout.TextField("Assets/Packages/", _packageName, GUILayout.Width(position.width - 20));

            var isShowNestedAssemblyDefOption = false;
            foreach (var entry in DataHandler.FolderAndFilesList.ToList())
            {
                var extension = entry.Key.Split(".").LastOrDefault();
                ...
                if (extension == "asmdef")
                {
                    ...
                    {
                        var entryKey = $"{entry.Key.Replace("[[packagename]]", $"{_packageName}")}";
                        DataHandler.FolderAndFilesList[entry.Key] = EditorGUILayout.ToggleLeft(entryKey, entry.Value,
                            GUILayout.Width(position.width - 20));
                    }
                }
                else
                {
                    DataHandler.FolderAndFilesList[entry.Key] = EditorGUILayout.ToggleLeft(entry.Key, entry.Value);
                }
                ...
                else if (entry.Key == DataHandler.ReadmeMD || entry.Key == DataHandler.ChangelogMD ||
                         entry.Key == DataHandler.ThirdPartyNoticesMD)
                {
                    if (entry.Value)
                    {
                        var fileExist = Path.Combine(DataHandler.Root, _packageName, entry.Key);
                ...
                {
                    isShowNestedAssemblyDefOption = entry.Value;
                }
                else if (entry.Key == "Runtime")
                {
                    isShowNestedAssemblyDefOption = entry.Value;
                }
                else if (entry.Key == "Tests/EditMode")
                {
                    isShowNestedAssemblyDefOption = entry.Value;
                }
                else if (entry.Key == "Tests/PlayMode")
                {
                    isShowNestedAssemblyDefOption = entry.Value;
                }
            ...
            if (!string.IsNullOrEmpty(_warningMessage) &&
                Time.realtimeSinceStartup - _warningStartTime < WarningDuration)
        ...
        private void CreateFileAsmdef(string packageDirectory, string fileName)
        {
            var filePath = Path.Combine(packageDirectory,
                $"{fileName.Replace("[[packagename]]", $"{DataHandler.GetSavedPackagedName()}")}");
            ...
            if (assemblyDefinitionContent != "")
                ...
                Debug.LogError("Assembly Definition is not created!");
        }
